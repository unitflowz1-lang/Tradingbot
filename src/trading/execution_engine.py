...
from src.runtime.emergency_stop import emergency_stop_active, register_emergency_stop_callback
...
class ExecutionEngine(TradeExecutor):
    ...
    _registered_engines: "weakref.WeakSet[ExecutionEngine]" = weakref.WeakSet()
    ...
    def __init__(self, broker: BrokerInterface, config: Optional[BrokerConfig] = None):
        ...
        # AutoTrading disabled HOLD_MODE
        ...
        self.__class__._registered_engines.add(self)
        register_emergency_stop_callback(
            f"execution_engine:{id(self)}",
    ...
    def _check_autotrading_enabled(self) -> bool:
        ...
        try:
            ...
            if terminal_info is None:
                self.logger.warning("[AUTOTRADING_CHECK] terminal_info() returned None")
                return True  # Can't check, assume OK
            ...
            self.logger.warning("[AUTOTRADING_CHECK_ERROR] Failed to check terminal state: %s", str(e))
            return True  # Can't check, assume OK

    def _enter_hold_mode(self, reason: str, hold_duration_seconds: int = 60) -> None:
        """
        Enter HOLD_MODE to stop all execution until AutoTrading is re-enabled.

        Args:
            reason: Reason for entering HOLD_MODE
            hold_duration_seconds: How long to wait before checking again
        """
        self._hold_mode_active = True
        self._hold_mode_until = datetime.now(timezone.utc) + timedelta(seconds=hold_duration_seconds)
        self.logger.critical(
            "[HOLD_MODE_ACTIVATED] AutoTrading disabled. Stopping all execution. "
            "Reason: %s | Will re-check at: %s",
            reason,
            self._hold_mode_until.strftime("%Y-%m-%d %H:%M:%S UTC"),
        )

    async def _handle_autotrading_disabled_error(self, order: Order) -> ExecutionResult:
        """
        Handle error 10027 (AutoTrading disabled).

        Steps:
        1. Log critical error
        2. Enter HOLD_MODE (stop all execution)
        3. Wait 60 seconds
        4. Re-validate terminal_info
        5. Flush execution queue for affected symbol

        Args:
            order: Order that triggered the error

        Returns:
            ExecutionResult with failure status
        """
        self.logger.critical(
            "[CRITICAL_AUTH_FAIL] AutoTrading is disabled in MT5 GUI. "
            "Enable it to proceed. Symbol: %s | Order ID: %s",
            order.symbol,
            order.order_id,
        )
        ...
        self._enter_hold_mode(
            reason=f"Error 10027: AutoTrading disabled for {order.symbol}",
            hold_duration_seconds=60
        )

        # Wait 60 seconds before re-validation
        self.logger.info("[TERMINAL_RESYNC] Waiting 60 seconds before re-validation...")
        await asyncio.sleep(60)

        # Re-validate terminal state
        self.logger.info("[TERMINAL_RESYNC] Checking if AutoTrading is re-enabled...")
        if self._check_autotrading_enabled():
            self._hold_mode_active = False
            self._hold_mode_until = None
            self.logger.info("[TERMINAL_RESYNC_SUCCESS] AutoTrading re-enabled. Resuming execution.")
        else:
            self.logger.critical(
        ...
        return ExecutionResult(
            ...
            error_message="CRITICAL_AUTH_FAIL: AutoTrading disabled (10027)",
            timestamp=datetime.now(timezone.utc),
        )

    async def purge_execution_queue_for_symbol(self, symbol: str, queue_processor) -> None:
        """
        Flush execution queue for a specific symbol when AutoTrading is disabled.
        This prevents stale trade signals from being executed after re-enabling.

        Args:
            symbol: Symbol to purge from queue
            queue_processor: RuntimeExecutionQueueProcessor instance (if available)
        """
        try:
            if queue_processor is None:
                self.logger.warning(
                    "[QUEUE_PURGE] Queue processor unavailable. Cannot purge queue for %s",
                    symbol
                )
                return

            # Access the queue from the processor (implementation may vary)
            # This is a placeholder - adjust based on actual queue structure
            symbol_key = self._symbol_key(symbol)

            self.logger.critical(
                "[QUEUE_PURGE] Flushing Execution_Queue for %s to prevent stale signals | "
                "AutoTrading was disabled, clearing expired trade signals.",
                symbol,
            )

            # If queue has a method to clear by symbol, call it
            if hasattr(queue_processor, 'clear_symbol_queue'):
                await queue_processor.clear_symbol_queue(symbol_key)

        except Exception as e:
            self.logger.error(
                "[QUEUE_PURGE_ERROR] Failed to purge queue for %s: %s",
                symbol,
                str(e)
            )
    ...
    async def execute(self, order: Order) -> ExecutionResult:
        ...
        if emergency_stop_active():
            self.logger.critical(
                "[EXECUTION_BLOCKED] %s | EMERGENCY_STOP active. Blocking order dispatch.",
            ...
            return ExecutionResult(
                ...
                executed_price=None,
                executed_quantity=None,
                error_message="EXECUTION_PAUSED: EMERGENCY_STOP_ACTIVE",
                timestamp=now,
            )
    ...
    def _get_max_allowable_pips(self, order: Order, avg_spread_pips: float) -> float:
        ...
        if hasattr(self.config, "max_spread_threshold") and getattr(self.config, "max_spread_threshold"):
            try:
                ...
                pass
        try:
            base_allowable = float(os.environ.get("MAX_ALLOWABLE_PIPS", "10.0"))
        except Exception:
            base_allowable = 10.0
        return round(max(base_allowable, float(avg_spread_pips) * 1.25), 2)
    ...
    async def _execute_market_order(self, order: Order, market_data: MarketData) -> ExecutionResult:
        ...
        try:
            ...
            if not self._check_autotrading_enabled():
                await self._handle_autotrading_disabled_error(order)
                return ExecutionResult(
                    success=False,
                    order_id=order.order_id,
                    executed_price=None,
                    executed_quantity=None,
                    error_message="CRITICAL_AUTH_FAIL: AutoTrading disabled in MT5 GUI",
                    timestamp=datetime.now(timezone.utc),
                )

            # ===== HOLD_MODE CHECK: Verify we're not in HOLD_MODE =====
            if self._hold_mode_active:
                if self._hold_mode_until and datetime.now(timezone.utc) < self._hold_mode_until:
                    self.logger.warning(
                        "[HOLD_MODE_ACTIVE] Execution blocked. %s | Resume at: %s",
                        order.symbol,
                        self._hold_mode_until.strftime("%Y-%m-%d %H:%M:%S UTC"),
                    )
                    return ExecutionResult(
                        ...
                        executed_price=None,
                        executed_quantity=None,
                        error_message="EXECUTION_PAUSED: HOLD_MODE_ACTIVE",
                        timestamp=datetime.now(timezone.utc),
                    )
                else:
                    # HOLD_MODE duration expired, try to exit
                    self.logger.info("[HOLD_MODE_EXPIRE] Duration expired. Re-checking AutoTrading state...")
                    if self._check_autotrading_enabled():
                        self._hold_mode_active = False
                        self._hold_mode_until = None
                        self.logger.info("[HOLD_MODE_EXIT] AutoTrading confirmed enabled. Resuming execution.")
                    else:
                        # Still disabled, extend HOLD_MODE
                        self._enter_hold_mode("AutoTrading still disabled after hold period")
                        return ExecutionResult(
                            success=False,
                            order_id=order.order_id,
                            executed_price=None,
                            executed_quantity=None,
                            error_message="EXECUTION_PAUSED: HOLD_MODE_EXTENDED",
                            timestamp=datetime.now(timezone.utc),
                        )
            ...
            if order.price is not None and market_data is not None:
                ...
                if favorable_slippage and slippage_pips > 0:
                    ...
                    # FIX: LOCK original TP to allow Realized RR to increase naturally.
                    # Do NOT "shrivel" profit target to maintain fixed ratio when market gives us better price.
                    old_tp = order.take_profit
                    favorable_move_pips = self._price_diff_to_pips(order.symbol, abs(execution_price - expected_price))
                    
                    # Calculate what the NEW RR would be if we keep original TP
                    risk_distance = abs(execution_price - order.stop_loss)
                    reward_distance = abs(old_tp - execution_price)
                    new_realized_rr = reward_distance / risk_distance if risk_distance > 0 else original_rr
                    rr_improvement = new_realized_rr - original_rr
                    
                    self.logger.critical(
                        f"[POSITIVE_SLIPPAGE_ACCEPT] {order.symbol} | Market moved favorably | "
                        f"Original RR: {format_float(original_rr, '.2f')}R | "
                        f"Realized RR (with locked TP): {format_float(new_realized_rr, '.2f')}R (+{format_float(rr_improvement, '.2f')}R) | "
                        ...
                        f"ACCEPTING IMPROVED ENTRY + LOCKING ORIGINAL TP"
                    )
                    
                    self.logger.critical(
                        f"[TP_LOCKED_POSITIVE_SLIPPAGE] {order.symbol} | Positive slippage detected: +"
                        f"{format_float(favorable_move_pips, '.2f')} pips | "
                        f"TP LOCKED at: {format_float(old_tp, '.5f')} (not extended) | "
                        f"RR increased from {format_float(original_rr, '.2f')}R to {format_float(new_realized_rr, '.2f')}R | "
                        f"Extra profit captured: +{format_float(rr_improvement, '.2f')}R"
                    )
                    # FIX: Do NOT adjust TP - keep original to capture extra RR from positive slippage
                    # order.take_profit remains unchanged
            ...
            try:
                ...
                # Capture the MT5 error code if available (e.g., 10018 Requote, 10019 Timeout, 10027 AutoTrading disabled)
                error_str = str(place_error)

                # ===== ERROR 10027: AUTOTRADING DISABLED =====
                if "10027" in error_str or "autotrading" in error_str.lower() or "trade_allowed" in error_str.lower():
                    self.logger.critical(
                        "[ERROR_10027] AutoTrading disabled detected during order placement | %s | %s",
                        order.symbol,
                        error_str
                    )
                    return await self._handle_autotrading_disabled_error(order)
    ...
    async def _handle_emergency_stop(self, reason: str) -> None:
        self.logger.critical(
            "[EXECUTION_EMERGENCY_STOP] %s | Clearing pending orders and blocking new execution.",
            reason,
        )
        self._hold_mode_active = True
        self._hold_mode_until = None
        try:
            await self.emergency_cancel_all_orders()
        finally:
