...
class EliteSignal:
    ...
    trade_tier: str = ""  # PRODUCTION: Tier classification (TIER_A, TIER_B, etc.)
    
    @property
    def is_tier_a(self) -> bool:
        """Check if this is a Tier-A (elite) signal"""
        # PRODUCTION FIX: Allow rotation if score > 80 (reduced from 90)
        return self.forced_execution or self.strategy_score >= 80.0 or self.trade_tier.upper() == "TIER_A"
    
    @property
    def tier_level(self) -> str:
        """Return tier classification"""
        if self.strategy_score >= 90.0 or self.trade_tier.upper() == "TIER_A":
            return "TIER_A_SCORE_90"
        elif self.strategy_score >= 80.0:
            return "TIER_A_SCORE_80"  # PRODUCTION: New tier level
...
class AutoRotationEngine:
    ...
    def should_rotate(self, 
                      ...
                      signal: EliteSignal,
                      open_positions: Optional[List[RotationCandidate]] = None) -> bool:
        """
        Determine if rotation should be triggered
        
        PRODUCTION FIX: Flexible rotation criteria
        Allow rotation if:
        A. New signal has Score > 80 (reduced from 90)
        B. There is an existing position with Score < 50 AND PnL < 0
        C. New signal belongs to 'Tier A' category
        
        Args:
            current_positions_count: Number of currently open positions
            signal: The incoming signal to evaluate
            open_positions: Optional list of current positions for underperformance check
        ...
        # PRODUCTION FIX: Check flexible rotation criteria
        # Criterion A: Signal score > 80
        if signal.strategy_score > 80.0:
            return True
        
        # Criterion B: Signal is Tier A
        if signal.trade_tier.upper() == "TIER_A":
            return True
        
        # Criterion C: Force execution
        if signal.forced_execution:
            return True
        
        # PRODUCTION FIX: Criterion D - Check if there's an underperforming position to replace
        # If signal score > existing position score AND existing position is losing
        if open_positions and signal.strategy_score >= 50.0:
            for pos in open_positions:
                # Check if position is underperforming (PnL < 0)
                if pos.unrealized_pnl < 0:
                    # We'll allow rotation if new signal is at least decent
                    self.logger.info(
                        "[ROTATION_FLEXIBLE] Signal %s (Score: %.1f) may replace underperforming %s "
                        "(PnL: $%.2f, Duration: %.1f min)",
                        signal.symbol,
                        signal.strategy_score,
                        pos.symbol,
                        pos.unrealized_pnl,
                        pos.open_duration_minutes,
                    )
                    return True
    ...
    def evaluate_rotation(self,
        ...
        # PRODUCTION FIX: Pass open_positions to should_rotate for flexible criteria
        if not self.should_rotate(len(open_positions), elite_signal, open_positions):
