...
class PositionManager:
    ...
    def __init__(self, broker: BrokerInterface, execution_engine: "TradeExecutor", promotion_callback=None):
        ...
        # Check for managed_tickets or initialize safe list (User Patch)
    ...
    def _build_shadow_payload_from_broker_position(self, position: Any) -> Dict[str, Any]:
        ...
        opened_at_iso = opened_at.isoformat() if isinstance(opened_at, datetime) else datetime.now(timezone.utc).isoformat()
        return {
    ...
    def _serialize_for_json(self, value: Any) -> Any:
        """
        FIX: Global JSON serialization helper - recursively converts datetime objects to ISO strings.
    ...
    def _save_state(self) -> None:
        """Save state to disk with global datetime serialization fix"""
        try:
            # FIX: Recursively serialize ALL datetime objects before json.dump
            serializable_attribution = self._serialize_for_json(self.position_attribution_data)
            serializable_partial_hits = {k: list(v) for k, v in self.partial_hits.items()}
            serializable_partial_hits = self._serialize_for_json(serializable_partial_hits)
            
            data = {
                'attribution_data': serializable_attribution,
            ...
            self.logger.debug(f"[STATE_SAVE_OK] State saved successfully to {self.STATE_FILE}")
        except Exception as e:
            self.logger.error(f"[STATE_SAVE_ERROR] Failed to save state: {e}", exc_info=True)
    ...
    def _sync_transactional_registry(self) -> None:
        ...
        HARD_REGISTRY_WIPE / CLEAR_GHOST_TICKETS guard:
        When either emergency flag is active the registry is treated as empty.
        No tickets are adopted and the file is zeroed in-place so the next
        normal startup also starts clean.
        """
        # ===== EMERGENCY BYPASS: respect HARD_REGISTRY_WIPE flag =====
        _hard_wipe = str(os.environ.get("HARD_REGISTRY_WIPE", "0")).strip().lower() in ("1", "true", "yes", "on")
        _clear_ghosts = str(os.environ.get("CLEAR_GHOST_TICKETS", "0")).strip().lower() in ("1", "true", "yes", "on")
        if _hard_wipe or _clear_ghosts:
            TRANSACTIONAL_REGISTRY_FILE = os.path.join(os.getcwd(), "transactional_tickets.json")
            try:
                if os.path.exists(TRANSACTIONAL_REGISTRY_FILE):
                    with open(TRANSACTIONAL_REGISTRY_FILE, 'r', encoding='utf-8') as _f:
                        _ghost_count = len(json.load(_f))
                    # Zero the file so a subsequent normal restart also starts clean
                    with open(TRANSACTIONAL_REGISTRY_FILE, 'w', encoding='utf-8') as _f:
                        json.dump({}, _f)
                    self.logger.critical(
                        "[REGISTRY_WIPE_SKIPPED] HARD_REGISTRY_WIPE is active. "
                        "Transactional registry recovery ABORTED. "
                        "%d ghost tickets purged from disk. "
                        "live_positions=0, registry_count=0.",
                        _ghost_count,
                    )
                else:
                    self.logger.critical(
                        "[REGISTRY_WIPE_SKIPPED] HARD_REGISTRY_WIPE is active. "
                        "No transactional_tickets.json found — registry already clean."
                    )
            except Exception as _wipe_err:
                self.logger.error(
                    "[REGISTRY_WIPE_ERROR] Failed to zero transactional_tickets.json during wipe: %s", _wipe_err
                )
            # Return immediately — do NOT adopt any tickets
            return
            ...
            if len(filtered_shadow) != len(self.shadow_positions):
                ...
                if hasattr(self, "managed_tickets"):
                    ...
                    if isinstance(_mt, dict):
                        ...
                        self.managed_tickets = clean_list
    ...
    def adopt_shadow_position(self, position_id: str, position_data: Dict, save_immediate: bool = True) -> None:
        ...
        sanitized_data.setdefault("target_rr", 2.0)
        sanitized_data["strategy_meta"].setdefault("target_rr", float(sanitized_data.get("target_rr", 2.0) or 2.0))
        ...
        # 3. Emergency SL & TP (Default if missing) - CRITICAL FOR ORPHAN TICKETS
        if 'emergency_sl' not in sanitized_data or sanitized_data['emergency_sl'] is None:
            # Default fallback: 50 pips for SL, 100 pips for TP (2:1 RR)
            direction = sanitized_data.get('direction', 0)
            entry = sanitized_data['entry_price']
            symbol_str = str(sanitized_data.get('symbol', '')).upper()
            
            # Determine pip value based on symbol
            if 'JPY' in symbol_str:
                pip_val = 0.01
            elif 'XAU' in symbol_str or 'GOLD' in symbol_str:
                pip_val = 0.10
            else:
                pip_val = 0.0001
            
            # Calculate ATR-based SL/TP if available, otherwise use fixed pips
            sl_pips = 50  # Default 50 pips
            tp_pips = 100  # Default 100 pips (2:1 RR)
            
            # Try to use ATR from strategy_meta if available
            try:
                atr_val = float(sanitized_data.get('strategy_meta', {}).get('symbol_report', {}).get('volatility', 0.0) or 0.0)
                if atr_val > 0:
                    sl_pips = atr_val * 10000  # Convert to pips
                    tp_pips = sl_pips * 2  # 2:1 RR
            except Exception:
                pass
            
            if direction in [0, Direction.LONG, 'LONG']:
                sanitized_data['emergency_sl'] = entry - (sl_pips * pip_val)
                sanitized_data['emergency_tp'] = entry + (tp_pips * pip_val)
            else:  # SHORT
                sanitized_data['emergency_sl'] = entry + (sl_pips * pip_val)
                sanitized_data['emergency_tp'] = entry - (tp_pips * pip_val)
            
            self.logger.critical(
                f"[ORPHAN_SL_TP_ASSIGN] Ticket {position_id} ({sym}) | "
                f"Direction: {'LONG' if direction in [0, Direction.LONG, 'LONG'] else 'SHORT'} | "
                f"Entry: {entry:.5f} | SL: {sanitized_data['emergency_sl']:.5f} | TP: {sanitized_data['emergency_tp']:.5f} | "
                f"RR: 2:1 | SL Distance: {sl_pips:.0f} pips"
            )
        
        # Ensure TP is also set even if SL exists
        if 'emergency_tp' not in sanitized_data or sanitized_data['emergency_tp'] is None:
            direction = sanitized_data.get('direction', 0)
            entry = sanitized_data['entry_price']
            sl = sanitized_data.get('emergency_sl', entry)
            symbol_str = str(sanitized_data.get('symbol', '')).upper()
            ...
            # Calculate distance from entry to SL, then apply 2x for TP
            sl_distance = abs(entry - sl)
            tp_distance = sl_distance * 2  # 2:1 RR
            
            if direction in [0, Direction.LONG, 'LONG']:
                sanitized_data['emergency_tp'] = entry + tp_distance
            else:  # SHORT
                sanitized_data['emergency_tp'] = entry - tp_distance
            
            self.logger.info(
                f"[ORPHAN_TP_ASSIGN] Ticket {position_id} ({sym}) | "
                f"TP assigned: {sanitized_data['emergency_tp']:.5f} (2:1 RR from SL)"
            )
            ...
            if isinstance(_mt, dict):
                ...
                for _item in _mt:
                    ...
                    if str(_it_tid) in active_ticket_ids:
                        ...
                        stale_managed_count += 1
                self.managed_tickets = _new_list
            elif isinstance(_mt, set):
                _old_len = len(_mt)
    ...
    async def sync_shadow_to_live(self, portfolio, entry_fn) -> int:
        if not self.use_shadow_state:
            return 0
        """
        Evaluate shadow positions and promote to live only after they prove
        at least +0.5R based on their own entry-to-stop risk distance.
        """
        promoted_count = 0
        promotion_r_threshold = 0.5
        ...
        for shadow_id, pos in list(self.shadow_positions.items()):
            ...
            stop_loss = float(pos.get('emergency_sl', 0.0) or 0.0)
            progress_r = 0.0
            if ep > 0 and current_price > 0 and stop_loss > 0 and stop_loss != ep:
                initial_risk = abs(ep - stop_loss)
                favorable_move = (current_price - ep) if is_long else (ep - current_price)
                progress_r = favorable_move / initial_risk

            promote_reason = None
            if progress_r >= promotion_r_threshold:
                promote_reason = f"Shadow PnL reached {progress_r:.2f}R >= {promotion_r_threshold:.2f}R"
            ...
            if promote_reason:
                ...
                self.logger.debug(
                    ...
                    f"Progress: {progress_r:.2f}R | Age: {age_secs:.0f}s | Needs {promotion_r_threshold:.2f}R"
                )
    ...
    def log_position_stats(self, portfolio: Dict) -> Dict[str, float]:
        ...
        # Calculate total spread costs and commissions from positions
        total_spread_cost = 0.0
        total_commission = 0.0
        for p in positions:
            total_spread_cost += float(((p.get('strategy_meta', {}) or {}).get("pos_spread_cost", 0.0)) or 0.0)
            total_commission += float(p.get('commission', 0.0) or 0.0)
        ...
        self.logger.info(
            ...
            f"Total Unr PnL: ${total_unrealized:.2f} (Incl. ${total_spread_cost:.2f} Spread) | "
            f"Commission: ${total_commission:.2f} | Total Costs: ${total_costs:.2f}"
        )
