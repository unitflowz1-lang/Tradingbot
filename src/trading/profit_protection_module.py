...
class DynamicProfitController:
    ...
    def __init__(
        ...
        self.levels = [
            {
                "target_r": float(self.settings.breakeven_trigger_r),
                "close_pct": 0.0,
                "action": "SET_BE",
                "state_key": "dynamic_be_primary",
            },
            {
                "target_r": float(self.settings.quick_profit_target_r),
                "close_pct": 0.5,
                "action": "PARTIAL_CLOSE",
                "state_key": "dynamic_partial_quick_profit",
            },
            {"target_r": 1.5, "close_pct": 0.0, "action": "ENABLE_TRAIL", "state_key": "dynamic_trail_1_5r"},
        ]

    def _resolve_levels(self, state: Dict[str, Any]) -> List[Dict[str, Any]]:
        regime_name = str(state.get("market_regime") or "").upper()
        quick_profit_target = float(self.settings.quick_profit_target_r)
        if regime_name == "RANGING":
            quick_profit_target = float(self.settings.ranging_quick_profit_target_r)
        resolved_levels: List[Dict[str, Any]] = []
        for level in self.levels:
            resolved_level = dict(level)
            if resolved_level.get("state_key") == "dynamic_partial_quick_profit":
                resolved_level["target_r"] = quick_profit_target
    ...
    async def check_exits(self, module: "ProfitProtectionModule", position: Position, state: Dict[str, Any], atr: float) -> bool:
        ...
        action_taken = False
        for level in self._resolve_levels(state):
    ...
    async def _execute_action(
        ...
        if action == "SET_BE":
            logger.info(
                "[DYNAMIC_PROFIT] %s (ID:%s) reached %.2fR. Moving SL to entry plus spread buffer.",
                position.symbol,
                pos_id,
                current_r,
            )
            ...
            if be_ok:
                ...
                state["last_modified_sl"] = float(getattr(position, "stop_loss", position.entry_price) or position.entry_price)
            return bool(be_ok)

        if action == "PARTIAL_CLOSE":
            close_pct = float(level.get("close_pct", 0.0) or 0.0)
            logger.info(
                "[DYNAMIC_PROFIT] %s (ID:%s) reached %.2fR. Closing %.0f%% to harvest gains.",
                position.symbol,
                pos_id,
                current_r,
                close_pct * 100.0,
        ...
        if action == "ENABLE_TRAIL":
            if not state.get("trailing_active", False):
                state["trailing_active"] = True
                logger.info(
                    "[DYNAMIC_PROFIT] %s (ID:%s) reached %.2fR. Enabling trailing protection.",
                    position.symbol,
                    pos_id,
                    current_r,
                )
            return bool(await module._apply_velocity_trailing(position=position, atr=atr, state=state, current_r=current_r))
...
class ProfitProtectionModule:
    ...
    async def manage_position(self, 
        ...
        if unrealized_pnl < 0:
            state['negative_pnl_cycles'] = int(state.get('negative_pnl_cycles', 0) or 0) + 1
        else:
            state['negative_pnl_cycles'] = 0
            state['force_close_requested'] = False
            state['force_close_reason'] = None

        negative_pnl_cycles = int(state.get('negative_pnl_cycles', 0) or 0)
        if extracted_ml_conf is not None:
            state['ml_confidence'] = float(extracted_ml_conf or 0.0)

        current_ml_conf_for_harvest = float(state.get('ml_confidence', 0.0) or 0.0)
        current_atr = 0.0
        if hasattr(market_data, 'atr'):
            current_atr = float(getattr(market_data, 'atr', 0.0) or 0.0)
        elif hasattr(market_data, 'indicators') and market_data.indicators:
            current_atr = float(market_data.indicators.get('atr', 0.0) or 0.0)
        elif isinstance(market_data, dict):
            current_atr = float(market_data.get('atr', 0.0) or 0.0)

        entry_buffer = abs(float(position.current_price or 0.0) - float(position.entry_price or 0.0))
        atr_buffer = max(0.0, current_atr * 0.5)
        ...
        if negative_pnl_cycles > int(self.settings.harvest_negative_pnl_max_cycles):
            if entry_buffer <= atr_buffer:
                ...
                state['force_close_reason'] = None
                logger.info(
                    "[HARVEST_BYPASS_DEFERRED] %s ID:%s | Negative for %d cycles but price is within 0.5 ATR "
                    "of entry (distance %.5f <= buffer %.5f).",
                    position.symbol,
            ...
            if current_ml_conf_for_harvest >= 0.30:
                ...
                state['force_close_reason'] = None
                logger.info(
                    "[HARVEST_SIGNAL_STILL_VALID] %s ID:%s | Negative for %d cycles but ML confidence %.1f%% "
                    "has not invalidated the trade.",
                    position.symbol,
                    position.position_id,
                    negative_pnl_cycles,
                    current_ml_conf_for_harvest * 100.0,
                )
                return False

            if not state.get('force_close_requested'):
                state['force_close_requested'] = True
                state['force_close_reason'] = (
                    f"HARVEST_BYPASS: Unrealized PnL stayed negative for {negative_pnl_cycles} cycles "
                    f"(threshold {self.settings.harvest_negative_pnl_max_cycles}) with ML confidence "
                    f"{current_ml_conf_for_harvest:.2f} below invalidation floor"
                )
                logger.critical(
                    "[HARVEST_BYPASS_CLOSE] %s ID:%s | Unrealized PnL negative for %d cycles, "
                    "price moved beyond 0.5 ATR buffer, and ML confidence %.1f%% < 30%%. "
                    "Harvest protection bypassed and position marked for closure.",
                    position.symbol,
                    position.position_id,
                    negative_pnl_cycles,
                    current_ml_conf_for_harvest * 100.0,
        ...
        if await self._execute_scale_out_and_lock(position, state, current_R):
            self._save_state()
            action_taken = True
        ...
        if current_R > 0:  # Only if in profit
            ...
            # This prevents the "Friday Suicide Loop" where trades are opened and immediately closed
            ...
            try:
                ...
                if position_opened_at is not None:
                    ...
                    time_since_open = (broker_now - position_opened_at).total_seconds() / 3600  # hours
                    
                    # Check if position was opened on a Friday
                    open_et_time = position_opened_at - timedelta(hours=5)  # Convert to ET
    ...
    async def _apply_dynamic_profit_lock(self, position: Position, state: Dict[str, Any]) -> bool:
        ...
        for level in self.settings.dynamic_profit_lock_levels:
            ...
            if current_profit_pct + 1e-9 < trigger_profit_pct:
                continue
