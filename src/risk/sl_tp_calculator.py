...
class StopLossTakeProfitCalculator:
    ...
    def __init__(self, atr_period: int = 14, risk_reward_ratio: float = 3.0, min_sl_pips: float = 3.0):
        """
    ...
    def calculate_levels(
        ...
        **IMPROVED:** Volatility-adjusted ATR multiplier for dynamic stop placement.
        ...
        calculated_sl_dist = float(sl_distance or 0.0) + (spread_buffer_pips * pip_value)
        min_distance = self.min_sl_pips * pip_value
        final_sl_dist = max(calculated_sl_dist, min_distance)

        if final_sl_dist > calculated_sl_dist:
            self.logger.warning(
                "[SL_DISTANCE_FLOOR] %s | ATR-derived SL %.5f too small. Applying %.1f pip floor -> %.5f",
                symbol,
                calculated_sl_dist,
                self.min_sl_pips,
                final_sl_dist,
        ...
        stop_loss = entry_price - final_sl_dist if direction == Direction.LONG else entry_price + final_sl_dist
        take_profit = refined_tp
        
        # Calculate Exact RR here
        raw_risk = abs(entry_price - stop_loss)
        raw_reward = abs(take_profit - entry_price)
        calculated_rr = 0.0
        if raw_risk > 0:
            calculated_rr = raw_reward / raw_risk
            
        self.logger.info(
            "[SLTP_CALC] %s | Computed exact price-based RR %.2fR from Entry=%.5f SL=%.5f TP=%.5f",
            symbol,
            calculated_rr,
            entry_price,
            stop_loss,
    ...
    def _calculate_ranging_take_profit(
        self,
        entry_price: float,
        direction: Direction,
        historical_data: List[MarketData],
    ) -> Optional[float]:
        """Target the center of the local range rather than a distant ATR multiple."""
        if not historical_data:
            return None

        closes = [float(bar.close) for bar in historical_data[-20:]]
        bb_mid = (sum(closes) / len(closes)) if closes else None

        range_window = historical_data[-24:] if len(historical_data) >= 24 else historical_data
        if not range_window:
            return bb_mid
        range_mid = (
            max(float(bar.high) for bar in range_window)
            + min(float(bar.low) for bar in range_window)
        ) / 2.0

        candidates = [target for target in (bb_mid, range_mid) if target is not None]
        if direction == Direction.LONG:
            valid_targets = [target for target in candidates if target > entry_price]
            return min(valid_targets) if valid_targets else None

        valid_targets = [target for target in candidates if target < entry_price]
        return max(valid_targets) if valid_targets else None
