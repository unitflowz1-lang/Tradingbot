...
MAX_SINGLE_TRADE_EXPOSURE_PCT = 5.0  # AGGRESSIVE SNIPER: Increased from 2.0 to 5.0 for TIER_A
...
class PositionSizingConfig:
    ...
    min_position_size: float = 0.001   # Minimum position size (0.1%)
    ...
    def validate(self) -> None:
        ...
        if not 0.0 < self.min_position_size <= 0.01:
            raise DataValidationError(
                f"Min position size must be between 0 and 0.01: {self.min_position_size}",
...
class PositionSizer(ABC):
    ...
    def _get_broker_min_lot(self) -> float:
        try:
            env_min = float(os.environ.get("BROKER_MIN_LOT", "0.01"))
        except Exception:
            env_min = 0.01
        cfg_min = float(getattr(self.config, "min_position_size", 0.01) or 0.01)
        return max(0.01, env_min, cfg_min)

    def _compute_proposed_exposure_pct(self, lots: float, account_balance: float) -> float:
        """
        Proposed exposure formula (CORRECTED):
        Exposure = (Lots * ContractSize) / AccountEquity * 100
        
        This gives the TRUE exposure as a percentage of account equity.
        Example: 0.01 lots * 100,000 / $10,000 equity = 0.10 (10%)
        
        Safety clamp: proposed_exposure = min(proposed_exposure, 0.02) [2% max per trade]
        Returned as percent (0-100 scale).
        """
        equity = float(account_balance or 0.0)
        volume = max(0.0, float(lots or 0.0))
        if equity <= 0.0 or volume <= 0.0:
            return 0.0
        # CORRECT FORMULA: (Volume * ContractSize) / Equity
        proposed_exposure = float((volume * CONTRACT_SIZE_UNITS) / equity)
        # Clamp to 2% max per single trade
        proposed_exposure = min(proposed_exposure, 0.02)
        return float(proposed_exposure * 100.0)
    ...
    def _validate_rr(self, signal: TradingSignal) -> bool:
        ...
        FIX: If technical alpha score > 80, lower minimum RR from 1.5 to 1.2.
        Returns False if RR < threshold, True otherwise.
        """
        # Get signal score to determine RR threshold
        ...
        # Use the higher of score or quality_score
        alpha_score = max(signal_score, quality_score)
        
        # FIX: If score > 80, lower RR threshold to 1.2 (Excellent signals shouldn't be skipped)
            ...
            if raw_risk > 0:
                ...
                if rr_ratio < rr_threshold:
                     logger.warning(
                         ...
                         f"Ratio {rr_ratio:.2f} < {rr_threshold:.1f} Minimum (Score: {alpha_score:.0f}). "
    ...
    def _confidence_multiplier(self, signal: TradingSignal) -> float:
        """
        AGGRESSIVE SNIPER: Remove confidence floor for Tier A signals.
        Scale position size UP for high-conviction signals (ML > 75%).
        """
        ...
        # AGGRESSIVE SNIPER: Boost high-conviction signals
        sig_tier = str(getattr(signal, "trade_tier", getattr(signal, "tier", "")) or "").upper()
        if confidence > 0.75:
            if sig_tier == "TIER_A":
                multiplier = min(confidence * 1.5, 1.5)
                logger.info(
                    f"[SNIPER_BOOST] {signal.symbol} | Elite signal (ML > 75%) | Confidence {confidence:.2f} -> {multiplier:.2f}x multiplier"
                )
                return multiplier
            elif sig_tier == "TIER_B":
                multiplier = min(confidence * 1.3, 1.3)
                logger.info(
                    f"[SNIPER_BOOST] {signal.symbol} | Strong signal (ML > 75%) | Confidence {confidence:.2f} -> {multiplier:.2f}x multiplier"
                )
                return multiplier
        
        # No floor - let confidence scale naturally (removed 0.50x Tier A floor)
        return confidence
    ...
    def _enforce_final_lot_floor(self, signal: TradingSignal, lots: float) -> float:
        """
        DEPRECATED: Use _apply_final_lot_floor instead.
        Kept for backward compatibility.
        """
        broker_min = self._get_broker_min_lot()
        if float(lots or 0.0) < broker_min:
            logger.critical(
                "[ZERO_SIZE_ABORT] %s | Final lot %.4f below broker minimum %.4f. Aborting signal.",
                getattr(signal, "symbol", "UNKNOWN"),
                float(lots or 0.0),
                broker_min,
            )
            raise SignalAbortedException(f"[ZERO_SIZE_ABORT] {getattr(signal, 'symbol', 'UNKNOWN')} | Final size {float(lots or 0.0):.4f} < {broker_min:.4f} lots")
        return float(lots or 0.0)
    
    def _apply_final_lot_floor(self, signal: TradingSignal, lots: float, account_balance: float) -> float:
        """
        FIX #5: SINGLE-POINT LOT FLOOR ENFORCEMENT
        
        This is the ONLY place where lot floors are checked, preventing multiple rounding jumps.
        All position sizing methods call this at the very end of their calculation chain.
        
        Checks performed (in order):
        1. Broker minimum lot size with signal quality-based recovery
        2. Account balance threshold floor (for accounts > $50k)
        3. Final validation and logging
        
        Args:
            signal: Trading signal
            lots: Calculated position size in lots
            account_balance: Current account balance
            
        Returns:
            Final position size after floor enforcement
        """
        sym = str(getattr(signal, "symbol", "UNKNOWN") or "UNKNOWN")
        broker_min = self._get_broker_min_lot()
        lots = float(lots or 0.0)

        # Hard cap: no single trade can exceed tier-based exposure of equity.
        sig_tier = str(getattr(signal, "trade_tier", getattr(signal, "tier", "")) or "").upper()
        if sig_tier == "TIER_A":
            max_exposure_pct = 5.0  # AGGRESSIVE: 5% for elite signals
        elif sig_tier == "TIER_B":
            max_exposure_pct = 3.0  # MODERATE: 3% for strong signals
        else:
            max_exposure_pct = 2.0  # CONSERVATIVE: 2% for normal signals
        
        max_lots_by_exposure = (float(account_balance or 0.0) * (max_exposure_pct / 100.0)) / CONTRACT_SIZE_UNITS
        if max_lots_by_exposure > 0.0 and lots > max_lots_by_exposure:
            logger.critical(
                "[EXPOSURE_CAP] %s | %s signal | Proposed exposure %.2f%% > %.2f%% cap. Resizing %.4f -> %.4f lots.",
                sym,
                sig_tier,
                self._compute_proposed_exposure_pct(lots, account_balance),
                max_exposure_pct,
                lots,
                max_lots_by_exposure,
            )
            lots = float(max_lots_by_exposure)
        
        # Step 1: Check broker minimum with signal quality-based recovery
        if lots < broker_min and lots > 0.0:
            # Extract signal quality score
            quality_score = float(getattr(signal, "quality_score", 0.0) or 0.0)
            confidence = float(getattr(signal, "confidence", 0.0) or 0.0)
            signal_score = float(getattr(signal, "score", getattr(signal, "signal_score", 0.0)) or 0.0)
            ...
            if quality_score <= 0.0:
                if confidence <= 1.0:
                    signal_quality = confidence * 100.0
                else:
                    signal_quality = confidence
            else:
                signal_quality = quality_score
            
            # FIX: Get trade tier to check if signal is admitted
            sig_tier = str(getattr(signal, "trade_tier", getattr(signal, "tier", "")) or "").upper()
            is_admitted = sig_tier in ("TIER_A", "TIER_B")
            
            # FIX: If signal is admitted (Tier A or B), CLAMP to broker minimum - NEVER abort
            if is_admitted or signal_quality > 35.0:
                logger.warning(
                    "[SIZE_RECOVERY_ADMITTED] %s | Calculated size %.4f below broker min %.4f but signal is %s (quality %.1f). Forcing to %.4f for execution.",
                    sym,
                    lots,
                    broker_min,
                    sig_tier if is_admitted else "QUALIFIED",
                    signal_quality,
                    broker_min,
                )
                lots = broker_min
            else:
                # FIX: If signal quality <= 35 and not admitted tier, abort silently
                logger.info(
                    "[SIZE_ABORT_LOW_QUALITY] %s | Calculated size %.4f below broker min %.4f and quality score %.1f <= 35. Aborting silently.",
                    sym,
                    lots,
                    broker_min,
                    signal_quality,
                )
                raise SignalAbortedException(f"[SIZE_ABORT_LOW_QUALITY] {sym} | Size {lots:.4f} < {broker_min:.4f}, quality {signal_quality:.1f}")
        
        # Step 2: REMOVED - Lot size mismatch abort logic disabled for Aggressive Sniper Mode
        # This prevents the bot from killing valid signals when risk model wants smaller size
        # Account threshold floor is still respected but no longer causes aborts
        
        # Step 3: Final validation
        final_lots = float(lots or 0.0)
        proposed_exposure_pct = self._compute_proposed_exposure_pct(final_lots, account_balance)
        logger.info(
            f"[FINAL_LOT_SIZE] {sym} | Final position size: {final_lots:.4f} lots | "
            f"Broker min: {broker_min:.4f} | Account: ${account_balance:.2f} | "
            f"Proposed Exposure: {proposed_exposure_pct:.2f}%"
        )
    ...
    def _apply_limits(self, position_size: float, account_balance: float, signal: TradingSignal, shadow_positions_pl: float = 0.0) -> float:
        """
        Apply position size limits with hard floor enforcement.
        
        ===== FIX #5: SINGLE LOT FLOOR APPLICATION =====
        The lot floor is now checked ONLY ONCE at the very end of the calculation chain
        to prevent multiple rounding jumps that cause conflicting lot sizes (0.16 vs 0.08 vs 0.10).
        
        ===== FIX #3: EXCELLENT TRADE SIZING FLOOR =====
        For signals with quality >= 0.75 (✓ EXCELLENT), enforce a MIN_FINAL_SIZE of 0.20x.
        This ensures high-conviction trades get meaningful position sizes regardless of regime penalties.
        ...
        """
        if position_size <= 0:
            logger.critical(f"[ZERO_SIZE_ABORT] {getattr(signal, 'symbol', 'UNKNOWN')} | Position size <= 0: {position_size}. Aborting signal.")
            raise SignalAbortedException(f"[ZERO_SIZE_ABORT] {getattr(signal, 'symbol', 'UNKNOWN')} | Position size {position_size:.4f} <= 0")
        
        baseline_size = float(position_size or 0.0)
        
        # Adjust available margin by shadow position unrealized losses
        adjusted_balance = account_balance + shadow_positions_pl  # PnL is negative for losses
        
        if adjusted_balance <= 0:
            # If shadow positions have already consumed the account, only allow minimal sizing
            return self.config.min_position_size
        
        # Apply maximum position size limit
        max_size = min(self.config.max_position_size, adjusted_balance * 0.95)  # Leave 5% buffer
        position_size = min(position_size, max_size)
        
        # ===== FIX #3: EXCELLENT SIGNAL SIZING FLOOR (ENHANCED) =====
        # If ML model + Technicals both label trade as "Excellent",
        # enforce MIN_SIZE_FLOOR of 0.25x to prevent regime throttling
        ...
        # Check for EXCELLENT label (score >= 55 AND confidence >= 0.75)
        excellent_floor = 0.25  # MIN_SIZE_FLOOR for elite signals
        is_excellent = (signal_score >= 55.0 or quality_metric >= 0.55) and quality_metric >= 0.75
        
        if is_excellent:
            # Elite signal: prevent regime_multiplier from throttling below 0.25x
            position_size = max(position_size, excellent_floor)
            logger.critical(
                f"[EXCELLENT_FLOOR] {getattr(signal, 'symbol', 'UNKNOWN')} | "
                f"Score={signal_score:.1f} | Quality={quality_metric:.2f} | "
                f"Calculated={baseline_size:.4f} | Floor={excellent_floor:.4f} | Final={position_size:.4f}"
            )
        
        # FIX #5: REMOVED intermediate lot floor check - will apply ONLY once at the end
        # This prevents multiple rounding jumps (0.16 -> 0.08 -> 0.10)
        ...
        # Log the sizing decision before final floor enforcement
        logger.info(
            f"[POSITION_SIZE_CALC] {getattr(signal, 'symbol', 'UNKNOWN')} | "
            f"Base: {baseline_size:.4f} | After multipliers: {position_size:.4f} lots"
        )
        
        # FIX #5: Apply ALL floor checks in a SINGLE final step
        # This prevents cascading floor applications that cause size jumps
...
class FixedFractionalSizer(PositionSizer):
    ...
    def calculate_position_size(  # type: ignore[override]
        ...
        try:
            ...
            position_size = position_units / CONTRACT_SIZE_UNITS
            ...
            lowest_multiplier = min(confidence_mult, tier_multiplier, volatility_multiplier)
            position_size *= lowest_multiplier
            
            logger.info(
                f"[POSITION_SIZING_CALC] {signal.symbol} | "
                f"Base Size: {position_units / CONTRACT_SIZE_UNITS:.4f} lots | "
            ...
            # FIX #5: Apply limits WITHOUT intermediate floor checks
            position_size = self._apply_limits(position_size, account_balance, signal)
            
            # FIX #5: Apply SINGLE final lot floor check at the very end
            # This is the ONLY place where broker minimum and account thresholds are enforced
            return self._apply_final_lot_floor(signal, position_size, account_balance)
            ...
            position_size = 0.0
        except SignalAbortedException as sae:
            # PRODUCTION FIX: Handle expected abort signals silently (no ERROR log)
            # These are intentional trade rejections (lot size mismatch, low quality, etc.)
            # Log as INFO instead of ERROR to reduce noise
            logger.info(
                f"[MARGIN_CALC_ABORTED] Trade aborted as expected: {sae}. "
                f"No position will be opened. This is normal risk management behavior."
            )
            position_size = 0.0
        ...
        # ===== FIX #6: ENFORCE 0.90X RANGING BOOST =====
        # If market regime is RANGING and confidence > 85%, deploy boosted capital
        ranging_boost_multiplier = 1.0
        if market_regime == 'RANGING' and confidence and confidence > 0.85:
            # Apply 0.90x boost for ranging markets with high confidence
            ranging_boost_multiplier = 0.90
            boosted_risk = risk_amount * ranging_boost_multiplier
            logger.critical(
                f"[DEPLOYING_BOOSTED_CAPITAL] {signal.symbol} in Ranging market with Confidence > 85%. "
                f"Boosted capital deployed: 0.90x floor active | "
                f"Base Risk: ${risk_amount:.2f} | Boosted Risk: ${boosted_risk:.2f}"
            )
        ...
        logger.critical(
            ...
            f"Position Size: {position_size:.4f} | Tolerance: 1.5 pips | "
            f"Confirms {scaled_risk * 100:.2f}% risk model active"
        )
        if position_size <= 0.0:
            logger.critical(
                f"[ZERO_SIZE_ABORT] {signal.symbol} | Position size locked to 0.0. Aborting signal."
            )
            raise SignalAbortedException(f"[ZERO_SIZE_ABORT] {signal.symbol} | Final position size 0.0")
...
class KellyCriterionSizer(PositionSizer):
    ...
    def calculate_position_size(
        ...
        logger.critical(
            f"[READY_TO_STRIKE] Risking ${risk_amount:.2f} on NEW_ORDER | {signal.symbol} | "
            f"Size: {position_size:.4f} | (Kelly Criterion)"
        )
        if position_size <= 0.0:
            logger.critical(
                f"[ZERO_SIZE_ABORT] {signal.symbol} | Kelly position size 0.0. Aborting signal."
            )
            raise SignalAbortedException(f"[ZERO_SIZE_ABORT] {signal.symbol} | Kelly size 0.0")
...
class AdaptivePositionSizer(PositionSizer):
    ...
    def calculate_position_size(
        ...
        combined_size = (kelly_weight * kelly_size) + (fixed_weight * fixed_size)
        ...
        logger.critical(
            f"[READY_TO_STRIKE] Risking ${risk_amount:.2f} on NEW_ORDER | {signal.symbol} | "
            f"Size: {combined_size:.4f} | (Adaptive blended)"
        )
        if combined_size <= 0.0:
            logger.critical(
                f"[ZERO_SIZE_ABORT] {signal.symbol} | Adaptive position size 0.0. Aborting signal."
            )
            raise SignalAbortedException(f"[ZERO_SIZE_ABORT] {signal.symbol} | Adaptive size 0.0")

        # FIX #5: Apply SINGLE final lot floor check at the very end
        return self._apply_final_lot_floor(signal, combined_size, account_balance)
