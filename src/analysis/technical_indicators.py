...
class IndicatorCalculator:
    ...
    def calculate_indicators(self, symbol: str, timeframe: str = '1h',
        ...
        """
        FIX #3: Calculate all technical indicators for a symbol with auto-scaling.
        If requested periods > available, automatically downscale to available - 50.
        """
        ...
        # FIX #3: Auto-scale periods request if not enough data available
        available_bars = len(self.data_cache[data_key])
        if periods > available_bars:
            # Auto-downscale to available - 50 (safety buffer)
            auto_scaled_periods = max(available_bars - 50, 20)  # Minimum 20 for indicators
            logger.warning(
                "[DATA_BUFFER_AUTO_SCALE] %s | Requested %d bars but only %d available | "
                "Auto-scaling to %d bars",
                symbol, periods, available_bars, auto_scaled_periods,
            )
            periods = auto_scaled_periods
            
        data_list = self.data_cache[data_key][-periods:]
            
        if len(data_list) < 20:  # Minimum for most indicators
            raise DataValidationError(
                f"Need at least 20 periods for indicators, got {len(data_list)}",
