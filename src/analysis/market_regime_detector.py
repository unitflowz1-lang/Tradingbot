...
class MarketRegimeDetector:
    ...
    def get_action(self, regime: str, vol_regime: str, signal_quality: float = 0.0) -> Dict[str, Any]:
        ...
        actions = {
            ...
            # ISSUE #1 FIX: No regime should return 0.00 if signal passes quality floor
            # RANGING/HIGH_VOL now uses minimum 0.5x multiplier for admitted trades
            ...
            ('RANGING', 'LOW_VOL'): {
                ...
                'size_multiplier': 0.8 if is_high_quality else 0.5,  # Min 0.5x floor (was 0)
                'reason': 'Ranging + Low vol (High quality: 0.8x, Standard: 0.5x)'
            },
            # FIX #3: Increased RANGING/HIGH_VOL multiplier from 0.5x to 0.75x for EXCELLENT signals
            # Ensures minimum 0.15x position size floor for high-quality trades
            ('RANGING', 'HIGH_VOL'): {
                'trade': True if is_high_quality else True,  # Always allow if passes admission
                'size_multiplier': 0.75 if is_high_quality else 0.6,  # Increased from 0.6/0.5 to 0.75/0.6
                'reason': 'Ranging + High vol (High quality: 0.75x, Standard: 0.6x)'
            },
            
            ('LOW_VOLATILITY', 'NORMAL_VOL'): {
                'trade': True if is_high_quality and quality >= 0.85 else True,
                'size_multiplier': 0.5 if is_high_quality else 0.5,  # Min 0.5x floor (was 0)
                'reason': 'Quiet market (Min 0.5x for admitted trades)'
            },
            ('LOW_VOLATILITY', 'LOW_VOL'): {
                'trade': True,  # Allow if passes admission
                'size_multiplier': 0.5,  # Min 0.5x floor (was 0)
                'reason': 'Consolidating (Min 0.5x for admitted trades)'
            },
            ('LOW_VOLATILITY', 'HIGH_VOL'): {
                'trade': True,  # Allow if passes admission
                'size_multiplier': 0.5,  # Min 0.5x floor (was 0)
                'reason': 'Whipsaw risk (Min 0.5x for admitted trades)'
            }
        }
        
        result = actions.get((regime, vol_regime), {'trade': True, 'size_multiplier': 0.5, 'reason': 'Unknown regime (min 0.5x floor)'})
    ...
    def detect_regime(self, price_data: List[Any]) -> str:
        """
        FIX #1: Alias method to match SimpleTrendStrategy calls.
        Detects regime from a list of MarketData objects (price_data).
        
        Args:
            price_data: List of MarketData objects (typically last 50-100 bars)
            
        Returns:
            Regime string: 'TRENDING', 'RANGING', or 'LOW_VOLATILITY'
        """
        try:
            if not price_data or len(price_data) < 20:
                return 'RANGING'  # Not enough data
            
            # Extract recent prices and calculate indicators
            closes = [float(getattr(bar, 'close', 0.0)) for bar in price_data[-50:]]
            if not closes or len(closes) < 20:
                return 'RANGING'
            
            # Calculate simple ADX proxy using price range
            high_prices = [float(getattr(bar, 'high', c)) for bar, c in zip(price_data[-50:], closes)]
            low_prices = [float(getattr(bar, 'low', c)) for bar, c in zip(price_data[-50:], closes)]
            ...
            atr_mean = sum(true_ranges) / len(true_ranges) if true_ranges else atr
            
            # Calculate ADX proxy using directional movement
            if len(closes) >= 28:
                # Simple trend strength measure
                recent_trend = abs(closes[-1] - closes[-14]) / (sum(closes[-14:]) / 14) * 100 if sum(closes[-14:]) > 0 else 0
                adx_proxy = min(recent_trend * 10, 50)  # Scale to ADX-like range (0-50)
            else:
                adx_proxy = 15  # Default weak
            
            # Build data dict for regime detection
            data_dict = {
                'adx': adx_proxy,
                'atr': atr,
                'atr_mean': atr_mean,
            ...
            # Use existing get_regime method
            return self.get_regime(data_dict)
            
        except Exception as e:
            logger.warning(f"[REGIME_DETECT_ERROR] Failed to detect regime: {e}")
