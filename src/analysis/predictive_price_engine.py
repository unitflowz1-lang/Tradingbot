...
class PredictivePriceEngine:
    ...
    def evaluate(
        ...
        *,
        ...
        # PRODUCTION FIX: Change from penalty to hard veto for sweep traps
        trap_detected = False  # NEW: Track trap detection for hard veto
        if signal_direction == Direction.LONG:
            weighted_strength = float(bullish_sweep_strength) ** 1.5 if bullish_sweep else 0.0
            if bearish_sweep:
                # PRODUCTION FIX: Hard veto instead of penalty for BEARISH_SWEEP_TRAP on LONG
                trap_detected = True
                edge = -0.20  # Maximum negative edge
                sweep_log = (
                    "critical",
                    f"[PREDICTIVE_CHART] {symbol} | BUYING INTO BEARISH SWEEP TRAP | HARD VETO - Trap detected",
                )
            elif bullish_sweep:
            ...
            if bullish_sweep:
                # PRODUCTION FIX: Hard veto instead of penalty for BULLISH_SWEEP_TRAP on SHORT
                trap_detected = True
                edge = -0.20  # Maximum negative edge
                sweep_log = (
                    "critical",
                    f"[PREDICTIVE_CHART] {symbol} | SELLING INTO BULLISH SWEEP TRAP | HARD VETO - Trap detected",
                )
            elif bearish_sweep:
                boost = sweep_w if (bearish_ob_hit or bearish_fvg_hit) else sweep_w * 0.8
                edge -= boost * weighted_strength
                sweep_log = (
                    "info",
                    f"[PREDICTIVE_CHART] {symbol} | LIQUIDITY SWEEP | Edge applied: -{boost * weighted_strength:.2f}",
        ...
        # PRODUCTION FIX: Return trap_detected flag for hard veto
        return edge, size_mult, dynamic_rr, attribution, exit_plan, log_msg, trap_detected
