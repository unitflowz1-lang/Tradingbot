...
class EnhancedSignalConfig:
    ...
    max_spread_multiplier: float = 2.0  # BLOCK if spread > 2x average (SPREAD TRAP PROTECTION)
...
class EnhancedSignalValidator:
    ...
    def validate_signal(
        ...
        """
        # ===== TEMPORARY BYPASS: Allow all signals that passed EV gate =====
        # This bypass disables confluence/accuracy filters to test trading volume
        # Remove this block once trading is flowing and we want to re-enable quality filters
        if True:  # Set to False to re-enable quality scoring
            ...
            if broker_safe:
                # Approve all signals with valid prices
                confluence_score = ConfluenceScore(
                    ...
                    indicator_confluence=100.0,
                    risk_alignment=100.0,
                ...
                self.logger.info(
                    "[VALIDATOR_BYPASS] %s | BYPASSING all confluence/accuracy filters | Accepting all EV-admitted signals",
                    symbol,
                )
        ...
        if admission_locked:
            ...
            confluence_score = ConfluenceScore(
                ...
                mtf_score=100.0 if broker_safe else 0.0,
                volatility_score=100.0 if broker_safe else 0.0,
                momentum_score=100.0 if broker_safe else 0.0,
                liquidity_score=100.0 if broker_safe else 0.0,
                indicator_confluence=100.0 if broker_safe else 0.0,
                risk_alignment=100.0 if broker_safe else 0.0,
                signal_approved=broker_safe,
                rejection_reasons=[] if broker_safe else ["Broker-breaking price payload"],
                recommended_entry=broker_safe,
                position_size_multiplier=1.0,
            )
            self.logger.critical(
                "[ADMISSION_LOCKED_BYPASS] %s | BYPASSING all downstream validators | admission_locked=True | Tug-of-War prevented | APPROVED",
                signal.symbol,
            )
            return confluence_score
    ...
    def _analyze_liquidity(
        ...
        # ===== SPREAD TRAP PROTECTION (Aggressive SL Strategy) =====
        # Block entries if spread > 2x average spread (prevents tight SL from being triggered by spread spikes)
        spread_trap_detected = False
        if symbol in self.volume_history and len(self.volume_history[symbol]) > 10:
            # Calculate average spread from recent history
            avg_spread_pips = np.mean(self.volume_history[symbol][-20:]) if len(self.volume_history[symbol]) >= 20 else spread_pips
            spread_ratio = spread_pips / max(avg_spread_pips, 0.1)
            
            if spread_ratio > self.config.max_spread_multiplier:
                spread_trap_detected = True
                self.logger.warning(
                    f"[SPREAD_TRAP] {symbol} | Spread {spread_pips:.1f} pips > {self.config.max_spread_multiplier}x avg ({avg_spread_pips:.1f}) | "
                    f"BLOCKING ENTRY (protects tight 2.0 ATR SL)"
                )
        
        # Store current spread for future average calculation
        if symbol not in self.volume_history:
            self.volume_history[symbol] = []
        self.volume_history[symbol].append(spread_pips)
        self.volume_history[symbol] = self.volume_history[symbol][-50:]  # Keep last 50 readings
        ...
        score = 0.0
        if spread_acceptable and not spread_trap_detected:
            score += 0.3
        ...
        # BLOCK if spread trap detected
        if spread_trap_detected:
            score = 0.0  # Zero score blocks the trade
        
        return LiquidityFilter(
            spread_acceptable=spread_acceptable and not spread_trap_detected,
