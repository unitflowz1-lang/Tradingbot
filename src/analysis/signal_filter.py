...
class SignalFilter:
    ...
    def __init__(self, 
        ...
        self.criteria.min_confidence = 0.45
    ...
    @staticmethod
    def is_rollover_hibernation_window(server_time: Optional[datetime] = None) -> bool:
        """Return True during the toxic-spread rollover window using MT5 server time."""
        current_time = server_time or datetime.now(timezone.utc)
    ...
    @staticmethod
    def elite_spread_multiplier(signal_score: float) -> float:
        """High-conviction signals can tolerate slightly wider spread cost."""
        return 1.5 if float(signal_score or 0.0) > 85.0 else 1.0
    ...
    def filter_signals(self,
        ...
        if isinstance(market_data, dict):
            market_regime = str(
                market_data.get("market_regime", market_data.get("regime", market_data.get("market_mode", ""))) or ""
            ).upper()
            market_session = str(market_data.get("market_session", "") or "").upper()
        ...
        # ===== BUG #2 FIX: Assign min_conf_threshold to dynamically calculated floor =====
        min_conf_threshold = regime_confidence_floor  # Use dynamic floor instead of hardcoded 65%
        ...
        tokyo_session_guard_active = market_session == "TOKYO"
        session_adx_floor = 18.0 if tokyo_session_guard_active else 12.0
        required_adx_dynamic = session_adx_floor
        high_confidence_gate_active = False
        rule_source = "BASELINE"  # Default rule source
        if tokyo_session_guard_active:
            logger.info(
                "[TOKYO_VOLATILITY_GUARD] %s | Session=TOKYO | ADX floor hard-locked at %.1f",
                market_data.get("symbol", "UNKNOWN") if isinstance(market_data, dict) else "UNKNOWN",
                session_adx_floor,
            )
        ...
        if ml_confidence > 0.80:
            final_required_adx = max(session_adx_floor, required_adx_dynamic)
            logger.debug(
                f"[ADX_EXPLICIT_LOCK] ML Confidence {ml_confidence:.1%} > 80% | "
                f"final_required_adx = {final_required_adx:.1f} (Baseline 20.0 BYPASSED)"
            )
        else:
            final_required_adx = session_adx_floor
        final_required_adx = max(session_adx_floor, final_required_adx)
        ...
        for signal in remaining_signals:  # Process only remaining non-elite signals
            if self.is_symbol_on_loss_cooldown(signal.symbol):
                ...
                continue
            signal_ml_accuracy = float(
                getattr(signal, "ml_accuracy", getattr(self, "ml_accuracy_current", ml_confidence)) or 0.0
            )
            if signal_ml_accuracy < accuracy_floor:
                rejected_signals.append(
                    (signal, f"Rejected by ML_ACCURACY_FLOOR | {signal.symbol}: {signal_ml_accuracy:.3f} < {accuracy_floor:.3f}")
                )
            ...
            if signal_override_authorized or signal_ev_score > ev_threshold_r:
                logger.critical(
                    ...
                    f"EV_GATE approved (EV Score: {signal_ev_score:.2f}R, Threshold: {ev_threshold_r:.2f}R, Override: {signal_override_authorized}) | "
