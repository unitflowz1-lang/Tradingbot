...
@dataclass
class SignalWeights:
    """AGGRESSIVE SNIPER: Favor ML predictions over technical indicators"""
    sentiment_weight: float = 0.0      # Disable sentiment (reduces noise)
    technical_weight: float = 0.3      # Reduced from 0.6
    ml_weight: float = 0.7             # NEW: Heavy ML emphasis
    trend_confirmation_bonus: float = 0.15  # Increased bonus for alignment
...
class SignalCombiner:
    ...
    def __init__(self, 
                 ...
                 min_confidence_threshold: float = 0.45,  # AGGRESSIVE SNIPER: Lowered from 0.5 to 0.45
        ...
        self.override_price_action_score: float = 0.0
        self.override_pa_type: str = "NONE"
        ...
        # Walk-Forward Validator (anti-overfitting)
        from src.ml.wfo_validator import WFOValidator
        self.wfo_validator = WFOValidator(min_sharpe_ratio=1.8)
        self._wfo_last_check = None
        self._regime_sharpes = {"TRENDING": 1.8, "RANGING": 1.8, "CHOPPY": 1.8}  # Default values
    ...
    def _apply_adaptive_strictness(self, symbol: str, confidence: float, timestamp: datetime) -> float:
        """
        AGGRESSIVE SNIPER: Increase aggression during high-volatility sessions.
        London/New York overlap (8:00-12:00 EST) gets reduced quality floor.
        """
        # London/New York overlap: 8:00-12:00 EST (13:00-17:00 UTC)
        if 13 <= timestamp.hour <= 17:
            # High volatility session - boost confidence by 10%
            adjusted_confidence = min(confidence * 1.10, 1.0)
            logger.info(f"[ADAPTIVE_STRICTNESS] {symbol} | London/NY overlap | Confidence boosted: {confidence:.2f} -> {adjusted_confidence:.2f}")
            return adjusted_confidence
        
        # Asian session (low volatility): tighten by 5%
        if 0 <= timestamp.hour <= 8:
            adjusted_confidence = confidence * 0.95
            logger.info(f"[ADAPTIVE_STRICTNESS] {symbol} | Asian session | Confidence tightened: {confidence:.2f} -> {adjusted_confidence:.2f}")
            return adjusted_confidence
    ...
    def combine_signals(self,
        ...
        if not sentiment_result and not technical_signals:
            self.logger.critical(
                f"[COMBINER_NO_INPUTS] {symbol} | No signals to combine (sentiment=None, tech_signals=0)"
            )
        ...
        # AGGRESSIVE SNIPER: Apply adaptive strictness based on trading session
        if signal_direction is not None:
            confidence_score = self._apply_adaptive_strictness(symbol, confidence_score, reference_time or datetime.now(timezone.utc))
        
        # WFO STABILITY CHECK (anti-overfitting)
        if signal_direction is not None and hasattr(self, 'wfo_validator'):
            try:
                wfo_result = self.wfo_validator.validate_stability(
                    sharpe_trending=self._regime_sharpes.get("TRENDING", 1.8),
                    sharpe_ranging=self._regime_sharpes.get("RANGING", 1.8),
                    sharpe_choppy=self._regime_sharpes.get("CHOPPY", 1.8)
                )
                
                if not wfo_result.is_stable:
                    logger.warning(
                        f"[WFO_REJECT] {symbol} | Parameters unstable | "
                        f"Sharpe: {wfo_result.sharpe_ratio:.2f} < 1.8 | "
                        f"Stability: {wfo_result.stability_rank:.1f}/100"
                    )
                    return CombinedSignalResult(
                        trading_signal=None,
                        confidence_score=confidence_score,
                        combination_reasoning="WFO stability check failed - parameters overfitted",
                        contributing_signals=synchronized_signals,
                        risk_assessment="HIGH",
                        timestamp=reference_time or datetime.now(timezone.utc)
                    )
            except Exception as e:
                logger.warning(f"[WFO_CHECK_ERROR] {symbol} | WFO validation skipped: {e}")
        if (
            signal_direction is not None
            and (confidence_score >= min_conf_for_cycle or structure_force_admit)
        ):
            self.logger.critical(
                f"[COMBINER_SIGNAL_GATE_PASS] {symbol} | Direction: {signal_direction} | "
                f"Confidence: {confidence_score:.3f} >= Min: {min_conf_for_cycle:.3f} | "
                f"Creating trading signal..."
            )
            ...
            # Force-pass logic removed: low-confidence signals do not advance to admission.
            self.logger.critical(
                f"[COMBINER_SIGNAL_GATE_REJECTED] {symbol} | Direction: {signal_direction} | "
                f"Confidence: {confidence_score:.3f} < Min: {min_conf_for_cycle:.3f} | NO SIGNAL CREATED"
            )
            logger.warning(
            ...
            self.logger.critical(
                f"[COMBINER_NO_SIGNAL] {symbol} | Direction: {signal_direction} | "
                f"TechSignals: {len(technical_signals) if technical_signals else 0} | "
    ...
    def _calculate_combined_confidence(self,
        ...
        if ml_conf is not None and meta_prob is not None:
            ...
            if ml_conf_val is not None and meta_prob_val is not None:
                # AGGRESSIVE SNIPER: Elite signal weighting (>75% ML confidence)
                if ml_conf_val > 0.75:
                    combined = (ml_conf_val * 0.8) + (meta_prob_val * 0.2)  # Heavy ML weight
                    logger.info(f"[ELITE_WEIGHTING] ML confidence {ml_conf_val:.2f} > 75% | Applied 80/20 ML/Meta split")
                else:
                    combined = (ml_conf_val * 0.6) + (meta_prob_val * 0.4)  # Default
    ...
    def _create_trading_signal(self,
        ...
        if stop_loss is not None and take_profit is not None and current_price is not None:
            ...
            if risk > 0.00001:  # Avoid division by zero
                ...
                self.logger.warning(
                    f"[RR_CALC_FAILED] {symbol} | Risk too small ({risk:.6f}) to calculate RR. "
                    f"Setting RR=0.0 to prevent invalid ratios."
                )
                real_risk_reward_ratio = 0.0
        else:
            # ISSUE #3 FIX: Log when SL/TP are missing so RR drift can be traced
            self.logger.warning(
                f"[RR_CALC_MISSING_DATA] {symbol} | Cannot calculate RR: "
                f"stop_loss={stop_loss}, take_profit={take_profit}, current_price={current_price}. "
                f"RR will be 0.0 — check SLTPCalculator output."
            )
            real_risk_reward_ratio = 0.0
        ...
        fallback_reason = str(getattr(self, "_ml_fallback_reason_for_cycle", "") or "")
        if fallback_reason and fallback_reason != "model_trained":
            capped_multiplier = min(float(admission.final_position_multiplier or 0.0), 0.20)
            if capped_multiplier < float(admission.final_position_multiplier or 0.0):
                self.logger.warning(
                    "[HEURISTIC_SIZE_CAP] %s | FallbackReason=%s | Capping multiplier %.2fx -> %.2fx",
                    symbol,
                    fallback_reason,
                    float(admission.final_position_multiplier or 0.0),
                    capped_multiplier,
                )
                admission.final_position_multiplier = capped_multiplier
                final_position_size = np.clip(position_size * admission.final_position_multiplier, 0.01, 1.0)
        ...
        trading_signal = TradingSignal(
            ...
            price_action_score=float(getattr(self, "override_price_action_score", 0.0) or 0.0),
            pa_type=str(getattr(self, "override_pa_type", "NONE") or "NONE"),
