...
class PriceMovementPredictor:
    ...
    def __init__(self, symbol: str, horizon: int = 3, calibration_method: str = "sigmoid"):
        ...
        self._last_heuristic_cycle_id: Optional[int] = None
        self._last_heuristic_prediction: Optional[Tuple[int, float, Dict[str, float]]] = None
        self.last_inference_timestamp: Optional[str] = None
        self.cached_ml_signal: Optional[Tuple[int, float, Dict[str, float]]] = None
    ...
    def prepare_features(
        ...
        """
        FIX #4: Convert market data and indicators into a feature matrix.
        Added Volatility Floor filter and USD/CAD-specific features.
        """
        ...
        ohlcv_df = pd.DataFrame(
            {
                "open": [float(bar.open) for bar in historical_data[:length]],
                "high": [float(bar.high) for bar in historical_data[:length]],
                "low": [float(bar.low) for bar in historical_data[:length]],
                "close": [float(bar.close) for bar in historical_data[:length]],
                "tick_volume": [
                    float(getattr(bar, "volume", getattr(bar, "tick_volume", 0.0)) or 0.0)
                    for bar in historical_data[:length]
                ],
            }
        )
        price_action_df = PriceActionMath.extract_features(ohlcv_df)

        # FIX #4: Detect if this is USD/CAD for special feature engineering
        is_usdcad = False
        if historical_data and len(historical_data) > 0:
            symbol_name = str(getattr(historical_data[0], 'symbol', '')).upper()
            is_usdcad = 'USD' in symbol_name and 'CAD' in symbol_name
        ...
        for i in range(length):
            ...
            pa = price_action_df.iloc[i] if not price_action_df.empty else {}
            atr_val = float(ind.atr or 0.0)
            atr_history.append(atr_val)
            
            # FIX #4: Volatility Floor - Skip bars where spread > 50% of ATR (too noisy)
            spread = float(getattr(m, 'spread', 0.0) or 0.0)
            if atr_val > 0 and spread > (0.5 * atr_val):
                self.logger.debug(
                    "[VOLATILITY_FLOOR_FILTER] Bar %d skipped | Spread %.1f > 50%% of ATR %.1f",
                    i, spread, atr_val,
                )
                continue  # Skip this noisy bar
            ...
            features = {
                ...
                "atr": atr_val,
                "atr_relative": (atr_val / float(m.close)) if float(m.close or 0.0) else 0.0,
                ...
                "body_ratio": float(pa.get("body_ratio", 0.0) or 0.0),
                "upper_wick_ratio": float(pa.get("upper_wick_ratio", 0.0) or 0.0),
                "lower_wick_ratio": float(pa.get("lower_wick_ratio", 0.0) or 0.0),
                "bullish_rejection_score": float(pa.get("bullish_rejection_score", 0.0) or 0.0),
                "bearish_rejection_score": float(pa.get("bearish_rejection_score", 0.0) or 0.0),
                "body_expansion_ratio": float(pa.get("body_expansion_ratio", 0.0) or 0.0),
                "momentum_strike": float(pa.get("momentum_strike", 0.0) or 0.0),
                "micro_trend_slope_5": float(pa.get("micro_trend_slope_5", 0.0) or 0.0),
                "micro_trend_slope_10": float(pa.get("micro_trend_slope_10", 0.0) or 0.0),
                "micro_trend_vector": float(pa.get("micro_trend_vector", 0.0) or 0.0),
                "log_return_1": float(pa.get("log_return_1", 0.0) or 0.0),
                "log_return_3": float(pa.get("log_return_3", 0.0) or 0.0),
                "volatility_norm_10": float(pa.get("volatility_norm_10", 0.0) or 0.0),
                "vpin_toxicity": float(pa.get("vpin_toxicity", 0.0) or 0.0),
                "kurtosis_14": float(pa.get("kurtosis_14", 0.0) or 0.0),
                "hurst_exponent_20": float(pa.get("hurst_exponent_20", 0.5) or 0.5),
                "rvi_14": float(pa.get("rvi_14", 50.0) or 50.0),
                "bullish_breakout_prob": float(pa.get("bullish_breakout_prob", 0.5) or 0.5),
                "bearish_breakout_prob": float(pa.get("bearish_breakout_prob", 0.5) or 0.5),
                "price_action_score": float(pa.get("price_action_score", 0.0) or 0.0),
            }
            
            # FIX #4: Add USD/CAD-specific features (Oil Correlation & CAD Relative Strength)
            # FIX #2: Initialize cad_momentum_10 outside conditional to prevent UnboundLocalError
            cad_momentum_10 = 0.0  # Default value
            
            if is_usdcad:
                # USD/CAD has been underperforming with raw RVI input, so keep the column
                # for schema stability but neutralize it and rely more on normalized volatility.
                features["rvi_14"] = 50.0
                features["usdcad_body_ratio_focus"] = float(features["body_ratio"]) * 1.5
                features["usdcad_tail_risk_focus"] = (
                    float(features["vpin_toxicity"]) * 1.35
                    + float(features["kurtosis_14"]) * 0.85
                )
                features["bearish_rejection_score"] = 0.0

                # Oil Correlation Proxy: USD/CAD typically inversely correlates with oil prices
                # We use recent price momentum as a proxy for oil-driven CAD strength
                if i >= 10:
                    cad_momentum_10 = (m.close - historical_data[i-10].close) / historical_data[i-10].close if historical_data[i-10].close != 0 else 0.0
                    features["cad_relative_momentum_10"] = cad_momentum_10
                    
                    # Oil correlation proxy: negative momentum in USD/CAD often means oil strength
                    features["oil_correlation_proxy"] = -cad_momentum_10  # Inverse relationship
                else:
                    features["cad_relative_momentum_10"] = 0.0
                    features["oil_correlation_proxy"] = 0.0
                
                # CAD Relative Strength: Compare to EUR/CAD proxy (using EUR/USD as reference)
                # This is a simplified proxy - ideally you'd fetch EUR/CAD data
                eur_usd_proxy = 1.08  # Default EUR/USD reference
                if i >= 5:
                    eur_move = 0.0  # Simplified: assume stable EUR/USD
                    cad_strength_vs_eur = cad_momentum_10 - eur_move
                    features["cad_relative_strength"] = cad_strength_vs_eur
                else:
                    features["cad_relative_strength"] = 0.0
            else:
                # FIX #2: Ensure features exist for non-USD/CAD symbols to prevent missing column errors
                features["cad_relative_momentum_10"] = 0.0
                features["oil_correlation_proxy"] = 0.0
                features["cad_relative_strength"] = 0.0
                features["usdcad_body_ratio_focus"] = 0.0
                features["usdcad_tail_risk_focus"] = 0.0
    ...
    def train(self, historical_data: List[MarketData], indicators_list: List[TechnicalIndicators], sample_weights: Optional[List[float]] = None):
        """
        FIX #1: Train primary calibrated direction model and secondary calibrated meta-model.
        
        Args:
            historical_data: Market data for training
            indicators_list: Technical indicators for each bar
            sample_weights: Optional sample weights for regime-weighted training (3x for regime-matched)
        """
        ...
        try:
            ...
            # Align weights to the exact supervised target index before NaN cleanup.
            if sample_weights is not None:
                sample_weights = np.array(sample_weights, dtype=float)
                if len(sample_weights) != len(targets):
                    self.logger.warning(
                        "[ML_WEIGHT_FIX] Sample weights length %d != targets length %d | Realigning to target index",
                        len(sample_weights),
                        len(targets),
                    )
                min_len = min(len(sample_weights), len(targets), len(X))
            ...
            if sample_weights is not None:
                ...
                if len(sample_weights) != len(X):
                    self.logger.critical(
                        "[ML_WEIGHT_CRITICAL] Final mismatch: weights=%d, features=%d | Truncating to match",
                        len(sample_weights),
                        len(X),
                    )
                    min_len = min(len(sample_weights), len(X))
                    sample_weights = sample_weights[:min_len]
                    X = X.iloc[:min_len]
                    y = y[:min_len]
                
                self.logger.info(
                    "[ML_WEIGHTED_TRAINING] Applied sample weights | Total samples: %d | "
                    "Weight range: [%.1f, %.1f] | Sum: %.0f",
                    len(sample_weights),
                    sample_weights.min(),
                    sample_weights.max(),
                    sample_weights.sum(),
            ...
            # FIX #1: Split sample weights to match data splits
            if sample_weights is not None:
                w_base = sample_weights[:base_end]
                w_meta = sample_weights[base_end:meta_end]
                w_test = sample_weights[meta_end:]
            else:
                w_base = w_meta = w_test = None
            ...
            # FIX #1: Pass sample weights to fit if available
            if w_base is not None:
                self.model.fit(X_base_scaled, y_base, sample_weight=w_base)
            else:
    ...
    def _prepare_latest_feature_row(
        ...
        # [ML_INPUT_DEBUG] Log feature preparation details
        self.logger.debug(
            "[ML_INPUT_DEBUG] %s | Features prepared: %s | Current price: %.6f | Historical bars: %d",
            self.symbol,
            list(row.keys()),
            float(m.close),
            len(historical_data),
        ...
        if expected:
            ...
            df = df[expected]
            self.logger.debug(
                "[ML_INPUT_DEBUG] %s | Reordered to expected feature shape: %s",
                self.symbol,
                expected,
            )
        
        result = df.replace({np.inf: 0.0, -np.inf: 0.0}).fillna(0.0)
        self.logger.debug(
            "[ML_INPUT_DEBUG] %s | Final df shape: %s | Cols: %d",
            self.symbol,
            result.shape,
            len(result.columns),
        )
    ...
    def predict_with_details(
        ...
        """
        # [FORCE_INFERENCE_ON_WARMUP] Do NOT block inference during warm-up/training phase
        # The bot may be in 0/15 trades (warm-up) but we still need ML predictions
        model_available = SKLEARN_AVAILABLE and self.model is not None and self.scaler is not None
        is_model_trained = getattr(self, 'is_trained', False)
        current_cycle_id = getattr(self, "_current_cycle_id", None)
        if current_cycle_id is None:
            current_cycle_id = getattr(self, "_bot_cycle_count", None)
        current_timestamp = None
        if historical_data:
            latest_bar_ts = getattr(historical_data[-1], "timestamp", None)
            if latest_bar_ts is not None:
                try:
                    if getattr(latest_bar_ts, "tzinfo", None) is None:
                        latest_bar_ts = latest_bar_ts.replace(tzinfo=timezone.utc)
                    current_timestamp = latest_bar_ts.isoformat()
                except Exception:
                    current_timestamp = str(latest_bar_ts)
        if (
            current_timestamp is not None
            and self.last_inference_timestamp == current_timestamp
            and self.cached_ml_signal is not None
        ):
            self.logger.debug(
                "[ML_INFERENCE_CACHE_HIT] %s | Reusing cached inference for %s",
                self.symbol,
                current_timestamp,
            )
            return self.cached_ml_signal
        
        if not is_model_trained or not model_available:
            if (
                current_cycle_id is not None
                and self._last_heuristic_cycle_id == current_cycle_id
                and self._last_heuristic_prediction is not None
            ):
                return self._last_heuristic_prediction
            # [SYNTHETIC_ML_SIGNAL_GENERATED] Using fallback heuristic (untrained model or unavailable)
            self.logger.info(
                "[SYNTHETIC_ML_SIGNAL_GENERATED] %s | is_trained=%s | sklearn_available=%s | model_exists=%s | Using fallback RSI+MACD heuristic",
                self.symbol,
                is_model_trained,
                SKLEARN_AVAILABLE,
            ...
            details = {
                ...
                "fallback_reason": "model_not_trained" if not is_model_trained else "model_unavailable",
            }
            self.logger.info(
                "[SYNTHETIC_ML_SIGNAL_RESULT] %s | direction=%s | confidence=%.4f | rsi=%s | macd=%s",
                self.symbol,
                "UP" if direction == 1 else "DOWN",
                confidence,
                getattr(indicators, 'rsi', 'N/A'),
                getattr(indicators, 'macd_histogram', 'N/A'),
            )
            result = (direction, confidence, details)
            if current_cycle_id is not None:
                self._last_heuristic_cycle_id = current_cycle_id
                self._last_heuristic_prediction = result
            if current_timestamp is not None:
                self.last_inference_timestamp = current_timestamp
                self.cached_ml_signal = result
            return result

        # [ML_INPUT_DEBUG] Log input shape before inference
        df = self._prepare_latest_feature_row(historical_data, indicators, bars_since_last_loss=bars_since_last_loss)
        self.logger.debug(
            "[ML_INPUT_DEBUG] %s | Input shape: %s | Rows: %d | Columns: %d | Dtypes: %s",
            self.symbol,
            df.shape,
            len(df),
            len(df.columns) if len(df) > 0 else 0,
            str(df.dtypes.to_dict()) if len(df) > 0 else "empty",
        )
        ...
        if df.empty:
            if (
                ...
                return self._last_heuristic_prediction
            # [SYNTHETIC_ML_SIGNAL_GENERATED] Empty dataframe, use fallback
            self.logger.warning(
                "[SYNTHETIC_ML_SIGNAL_GENERATED] %s | Feature dataframe is EMPTY after preparation | Using fallback signal (confidence=0.5)",
                self.symbol,
            )
            result = (1, 0.5, {
                "primary_prob_up": 0.5,
        ...
        try:
            # [ML_INFERENCE_ATTEMPT] Log raw inference before any post-processing
            X_scaled = self.scaler.transform(df)
            self.logger.debug(
                "[ML_INFERENCE_ATTEMPT] %s | X_scaled shape: %s | X_scaled[0][:5]: %s",
                self.symbol,
                X_scaled.shape,
                X_scaled[0][:5] if len(X_scaled) > 0 else "empty",
            )
            
            raw_proba = self.model.predict_proba(X_scaled)
            self.logger.info(
                "[ML_INFERENCE_ATTEMPT] %s | Raw model output (predict_proba): [DOWN: %.6f, UP: %.6f] | shape: %s",
                self.symbol,
                raw_proba[0, 0] if len(raw_proba) > 0 else 0.0,
                raw_proba[0, 1] if len(raw_proba) > 0 else 0.0,
                raw_proba.shape,
            )
            primary_prob_up = float(raw_proba[0, 1])
            ...
            if (
                ...
                return self._last_heuristic_prediction
            # [SYNTHETIC_ML_SIGNAL_GENERATED] Exception during inference, use fallback
            self.logger.error(
                "[SYNTHETIC_ML_SIGNAL_GENERATED] %s | Inference exception (fallback activated): %s | Traceback: %s",
                self.symbol,
                str(e),
                traceback.format_exc() if 'traceback' in dir() else "N/A",
            )
            result = (1, 0.5, {
                "primary_prob_up": 0.5,
                "directional_confidence": 0.5,
