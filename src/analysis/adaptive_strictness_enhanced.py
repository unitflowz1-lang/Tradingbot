...
class MarketCondition(Enum):
    ...
    TRENDING = "TRENDING"          # ADX >= 20 (Aggressive/Strict)
...
class EnhancedAdaptiveStrictnessController:
    ...
    def detect_market_condition(self, analysis_data: Dict[str, Any]) -> MarketCondition:
        ...
        - STRONG_TREND: ADX >= 20 or (ADX >= 18 and ATR increasing >12%)
        - WEAK_TREND: 12 <= ADX < 20
        ...
        strict_adx_threshold = float(os.environ.get("ADX_MIN_STRICT", "20") or "20")
        supportive_adx_threshold = max(12.0, strict_adx_threshold - 2.0)
        if adx >= strict_adx_threshold or (adx >= supportive_adx_threshold and atr_increasing):
