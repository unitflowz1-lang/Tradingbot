...
class MetricsSnapshot:
    ...
    pa_type: str = "NONE"
    price_action_direction: str = "HOLD"
...
class DecisionMatrixResult:
    ...
    technical_only_forced_relief: bool = False
    contrarian_scalp: bool = False
    position_multiplier_override: Optional[float] = None
    timestamp: datetime = field(default_factory=lambda: datetime.now(timezone.utc))
...
class DecisionMatrix:
    ...
    def get_governance_state(self, metrics: Optional[MetricsSnapshot] = None) -> Tuple[Action, float]:
        ...
        price_action_score = float(getattr(metrics, "price_action_score", 0.0) or 0.0)
        pa_type = str(getattr(metrics, "pa_type", "") or "").upper()
        high_impact_pending = ("HIGH_IMPACT_NEWS_PENDING" in macro_reason) and macro_risk >= 0.10
        elevated_macro = macro_risk >= 0.25
        if price_action_score >= 80.0 and pa_type == "MATH_REJECTION" and not (elevated_macro or high_impact_pending):
    ...
    def ingest_signal_context(
        ...
        """
        Populate decision-matrix inputs from a TradingSignal and/or market context object.
        """
        ...
        pa_score = price_action_score
        if pa_score is None:
            pa_score = _extract(signal, "price_action_score", None)
        if pa_score is None:
            pa_score = _extract(market_context, "price_action_score", 0.0)
        pa_kind = pa_type
        if pa_kind is None:
            pa_kind = _extract(signal, "pa_type", None)
        if pa_kind is None:
            pa_kind = _extract(market_context, "pa_type", "NONE")
        pa_direction = direction
        if pa_direction is None:
            pa_direction = _extract(signal, "price_action_direction", None)
        if pa_direction is None:
            pa_direction = _extract(signal, "direction", None)
        if pa_direction is None:
            pa_direction = _extract(market_context, "price_action_direction", "HOLD")

        snapshot.price_action_score = float(pa_score or 0.0)
        snapshot.pa_type = str(pa_kind or "NONE")
        snapshot.price_action_direction = str(pa_direction or "HOLD")
        return snapshot
    ...
    def evaluate(self, metrics: MetricsSnapshot) -> DecisionMatrixResult:
        ...
        pa_type = str(getattr(metrics, "pa_type", "") or "").upper()
        macro_risk = float(getattr(metrics, "macro_risk", 0.0) or 0.0)
        macro_reason = str(getattr(metrics, "macro_risk_reason", "") or "")
        has_macro_warning = macro_risk >= 0.10 or "HIGH_IMPACT" in macro_reason.upper() or "BEARISH" in macro_reason.upper()

        if price_action_score >= 85.0 and pa_type == "MATH_REJECTION" and has_macro_warning:
            result.risk_level = RiskLevel.ELEVATED
            result.primary_action = Action.MAINTAIN_CURRENT
            result.triggers = [
                f"Macro warning active: {macro_reason or 'Macro risk block'}",
                f"Contrarian scalp enabled by Price Action Score {price_action_score:.1f}",
            ]
            result.severity_score = 35.0
            result.confidence = min(1.0, 0.60 + (price_action_score / 200.0))
            result.conviction_multiplier = 1.0
            result.technical_only_forced_relief = True
            result.contrarian_scalp = True
            result.position_multiplier_override = 0.5
            result.recommendation = "PA Governor override: Contrarian scalp authorized at half-size"
            self.logger.critical(
                "[PA_GOVERNOR_OVERRIDE] Price Action Score %.1f dominates. Elevating execution conviction.",
                price_action_score,
            )
            self._log_decision(result)
            return result

        if price_action_score >= 80.0 and pa_type == "MATH_REJECTION":
            result.risk_level = RiskLevel.OPTIMAL
            result.primary_action = Action.AGGRESSIVE_ENGAGEMENT
            result.triggers = [
                f"Price Action Score {price_action_score:.1f} >= 80.0",
                f"Pattern type {pa_type} promoted to portfolio governor",
            ]
            result.severity_score = 10.0
            result.confidence = min(1.0, 0.75 + ((price_action_score - 80.0) / 80.0))
            result.conviction_multiplier = 1.25
            result.recommendation = "PA Governor override: Mathematical rejection promoted to aggressive engagement"
            self.logger.critical(
                "[PA_GOVERNOR_OVERRIDE] Price Action Score %.1f dominates. Elevating execution conviction.",
                price_action_score,
            )
    ...
    def get_position_multiplier(self, base_multiplier: float, last_decision: Optional[DecisionMatrixResult] = None) -> float:
        ...
        conviction_multiplier = float(getattr(last_decision, "conviction_multiplier", 1.0) or 1.0)
        multiplier *= conviction_multiplier

        override = getattr(last_decision, "position_multiplier_override", None)
        if override is not None:
            override = float(override)
            multiplier = base_multiplier * override if override <= 2.0 else override
        
        # Clamp to valid range [0, 2.0] so PA conviction can pass through to sizing.
