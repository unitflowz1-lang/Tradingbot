...
class AlgoTradingBot:
    ...
    async def initialize(self, skip_broker: bool = False):
        ...
        try:
            # Initialize broker (use mock by default for safety)
            if skip_broker:
...
async def main():
    ...
    try:
        # Initialize with mock broker for safety
        if await bot.initialize(skip_broker=True):
