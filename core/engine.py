...
from src.trading.dynamic_trailing_sl_manager import (
    ...
    TrailingConfig as TrailingSLConfig,
)
...
class TradingEngine:
    ...
    def __init__(self, config: TradingConfig, broker: Broker, data_handler: DataHandler):
        ...
        # Initialize trailing SL manager - DISABLED to prevent SL from moving
        # HARD-CODED DISABLE: Set to None to completely bypass trailing SL logic
        trailing_config = TrailingSLConfig(
                ...
                for trade_id, trade in list(self.portfolio.trades.items()):
                    if trade.symbol == symbol:
                        ...
                        if trade.direction == "BUY" and current_price <= trade.stop_loss:
                            ...
                            # Untrack from trailing SL
                            state = self.trailing_sl_manager.untrack_position(str(trade_id))
                            if state:
                                logger.info(
    ...
    async def shutdown(self):
        ...
        for trade_id in list(self.portfolio.trades.keys()):
            ...
            # Untrack from trailing SL
            self.trailing_sl_manager.untrack_position(str(trade_id))
